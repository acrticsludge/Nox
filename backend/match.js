...
function snapshotOf(m) {
  return {
    ...
    score: m.scores,
    time: r1(m.timeLeft),
    p: m.players.map(p => [r1(p.x), r1(p.y), r1(p.angle), p.hp, p.ammoType, p.shield ? p.shieldHp : 0]),
    b: m.bullets.map(b => [r1(b.x), r1(b.y), Math.round(b.vx * 10) / 10, Math.round(b.vy * 10) / 10, b.type]),
