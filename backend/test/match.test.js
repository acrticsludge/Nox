...
function makeClient(nick) {
  ...
  return { ws, box, nick, next(type, timeout = 4000) {
    return new Promise((resolve, reject) => {
      const to = setTimeout(() => reject(new Error('timeout: ' + type)), timeout);
      const i = box.findIndex(m => m.type === type);
      if (i !== -1) { clearTimeout(to); resolve(box.splice(i, 1)[0]); return; }
      const iv = setInterval(() => {
        const j = box.findIndex(m => m.type === type);
        if (j !== -1) { clearTimeout(to); clearInterval(iv); resolve(box.splice(j, 1)[0]); }
      }, 20);
