...
// 1:1 preview — uses the SAME constants + rendering as frontend/src/game/game-logic.js
// No approximations: ship shape, bullet size/speed/cd/filters, pickup icons, hazards all match the real arena
export default function HeroArenaPreview() {
  ...
  useEffect(() => {
    ...
    // Walls — same algorithm as game (border + random inners with gap 34)
    ...
    // seed hazards same as game (4-6 random, not on walls/protected)
    {
      const occ = new Set<string>()
      walls.forEach(w=>{}); // placeholder
      // reuse gen logic: pick 5 hazards deterministic-ish
      const kinds: ('lava'|'slime')[] = ['lava','lava','slime','slime','lava']
      const spots = [[12,6],[18,9],[5,10],[17,3],[9,7]]
      spots.forEach(([c,r],i)=>{
        hazards.push({ x: c*GRID+2, y: r*GRID+2, w:36, h:36, kind: kinds[i % kinds.length], t: Math.random()*300 })
      })
    }
    ...
    const shoot = (pIdx:number, type:string) => {
      ...
      const dmg = type==='trick' ? 2.5 : type==='cannon' ? 4 : type==='needle' ? 0 : 2
      bullets.push({ x:mx,y:my, vx:Math.cos(ang)*cfg.speed, vy:Math.sin(ang)*cfg.speed, owner:pIdx, type, r:cfg.r, life:cfg.life, trail:[], bounces:0, dmg })
    ...
    const frame = () => {
      ...
      if(loopT<16){
        ...
        voidR=420; safeR=999; shootCd=[0,0]; dashCd=[0,0]; voidTick=0
      }
      ...
      players.forEach((p,i)=>{
        const other=players[1-i]; const tgt=targets[i]
      ...
      for(let i=bullets.length-1;i>=0;i--){
        ...
        if(hitWall){
          if(b.type==='trick' && b.bounces<5){
            ...
            for(const w of walls){ if(!rectCircle(b.x,b.y,b.r+1,w.x,w.y,w.w,w.h)) continue; const cx=clamp(b.x,w.x,w.x+w.w), cy=clamp(b.y,w.y,w.y+w.h); const d=Math.hypot(b.x-cx,b.y-cy); if(d<best){best=d; nx=b.x-cx; ny=b.y-cy} }
        ...
        for(const p of players){
          if(p.inv>0) continue; if(b.owner===(p===players[0]?0:1)) continue
      ...
      // DRAW pickups — same as game (glow + inner + icon + 3 dots + label)
      if(gPick){
        let s=''
      ...
      // DRAW bullets — same as game (trail + typed shape + filter)
      if(gBul){
        let b=''
      ...
      // DRAW players — SAME ship shape as game (M 18 0 L -12 -11 L -8 0 L -12 11 Z)
      if(gPlay){
        let p=''
        for(let i=0;i<players.length;i++){
          ...
          if(pl.shield){
            const hp=pl.shieldHp||5, ratio=hp/5; let dash='6 3', o='0.92', sw='2.6'
          ...
          p+=`</g>`
          // hp text (keep world-rotated)
          const hpTxt = pl.id===0 ? '12♥' : '12♥' // preview keeps full HP for demo
      ...
      if(gPart){
        let s=''
        for(const pt of particles){ const a=pt.life/pt.max; s+=`<circle cx="${pt.x}" cy="${pt.y}" r="${pt.r*a}" fill="${pt.col}" opacity="${a}"/>` }
        gPart.innerHTML=s
      }
    }
    frame()
