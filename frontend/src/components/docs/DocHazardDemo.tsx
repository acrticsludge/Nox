...
export function DocWallDemo() {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (now - last > 900) {
        // alternate trick bounce vs standard block
        const isTrick = Math.random() > 0.5
        bullets.push({ x: 44, y: 70 + (Math.random() * 20 - 10), vx: isTrick ? 6.2 : 7.2, bounces: 0, type: isTrick ? 'trick' : 'standard' })
        last = now
      }
      ...
      for (const b of bullets) {
        if (b.type === 'trick') h += `<path d="M ${b.x} ${b.y - 4} L ${b.x + 4} ${b.y} L ${b.x} ${b.y + 4} L ${b.x - 4} ${b.y} Z" fill="#fff" stroke="#58d8ff" stroke-width="1.6"/>`
        else h += `<circle cx="${b.x}" cy="${b.y}" r="5" fill="#fff" stroke="#58d8ff" stroke-width="2"/><circle cx="${b.x}" cy="${b.y}" r="2" fill="#a9e9ff"/>`
      }
      gBul.innerHTML = h
    }
    ...
    <div ref={ref} className="docs-demo">
      <div className="docs-demo__frame">
        <div className="docs-demo__top">
          ...
          <span className="docs-demo__note">Blocks most • trick bounces</span>
        </div>
...
export function DocLavaDemo() {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      g.innerHTML = `
        <circle cx="180" cy="70" r="${r}" fill="url(#ld-lava)" opacity="${active ? 1 : warn ? 0.78 : 0.42}" stroke="${active ? '#fff' : '#fb923c'}" stroke-width="${active ? 1.4 : 1}" ${active ? 'filter="url(#ld-soft)"' : ''}/>
        ...
        <g transform="translate(180,118)"><text text-anchor="middle" font-size="7" font-family="JetBrains Mono, monospace" fill="rgba(201,255,47,0.8)">${active ? 'BURNS • 2 HP' : warn ? 'BLINKS • WARNING' : 'COOLS'}</text></g>
      `
    }
    loop()
    return () => cancelAnimationFrame(raf)
  ...
  return (
    <div ref={ref} className="docs-demo">
      <div className="docs-demo__frame">
        <div className="docs-demo__top">
          <span className="docs-demo__badge" style={{ background: '#fb923c', borderColor: '#fb923c', color: '#07090b' }}>
            LAVA VENT
          </span>
          <span className="docs-demo__note">Blinks → burns</span>
        </div>
        <svg viewBox="0 0 360 140" xmlns="http://www.w3.org/2000/svg" className="docs-demo__svg">
          <defs>
...
export function DocSlimeDemo() {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const spd = inSlime ? 0.3 : 0.9
      x += spd
      gShip.setAttribute('transform', `translate(${x},70) ${inSlime ? 'scale(0.96)' : ''}`)
      gShip.innerHTML = `
        <ellipse cx="2" cy="10" rx="14" ry="6" fill="rgba(0,0,0,0.35)"/>
        <path d="M 18 0 L -12 -11 L -8 0 L -12 11 Z" fill="${inSlime ? '#6ee7b7' : '#3ec5f2'}" stroke="#fff" stroke-width="1.6" ${inSlime ? 'opacity="0.9"' : ''}/>
      ...
      if (txt) txt.textContent = inSlime ? 'INSIDE SLIME → SLOW' : 'OUTSIDE → NORMAL'
    }
    loop()
    return () => cancelAnimationFrame(raf)
  }, [])
  return (
    <div ref={ref} className="docs-demo">
      <div className="docs-demo__frame">
        <div className="docs-demo__top">
          <span className="docs-demo__badge" style={{ background: '#6ee7b7', borderColor: '#6ee7b7', color: '#07090b' }}>
            SLIME
          </span>
          <span id="sd-label" className="docs-demo__note">
            OUTSIDE → NORMAL
          </span>
        </div>
        <svg viewBox="0 0 360 140" xmlns="http://www.w3.org/2000/svg" className="docs-demo__svg">
          ...
          <text x="180" y="104" textAnchor="middle" fontSize="7" fontFamily="JetBrains Mono, monospace" fill="rgba(255,255,255,0.55)">
            SLIME • NO DAMAGE
          </text>
          <g id="sd-ship" />
        </svg>
...
export function DocVoidDemo() {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      // ship stays inside early, then gets pushed
      const d = 40 + Math.sin(now * 0.001) * 10
      const ang = Math.atan2(70 - 70, 180 - d - 180)
      const push = r < 90 && d > r - 16 ? r - 16 : d
      const x = 180 + Math.cos(0) * push * 0.3
      gShip.setAttribute('transform', `translate(${x},70)`)
      gShip.innerHTML = `<path d="M 18 0 L -12 -11 L -8 0 L -12 11 Z" fill="${loopT > 7000 ? '#c9ff2f' : '#3ec5f2'}" stroke="#fff" stroke-width="1.6"/>`
    }
    loop(performance.now())
    return () => cancelAnimationFrame(raf)
  }, [])
  return (
    <div ref={ref} className="docs-demo">
      <div className="docs-demo__frame">
        <div className="docs-demo__top">
          <span className="docs-demo__badge" style={{ background: '#c9ff2f', borderColor: '#c9ff2f', color: '#07090b' }}>
            THE VOID
          </span>
          <span className="docs-demo__note">After 45s • closes 420→110</span>
        </div>
        <svg viewBox="0 0 360 140" xmlns="http://www.w3.org/2000/svg" className="docs-demo__svg">
          <defs>
            ...
            <pattern id="vd-blocks" width="24" height="24" patternUnits="userSpaceOnUse"><rect width="12" height="12" fill="#1a0b2e" opacity="0.52"/><rect x="12" y="12" width="12" height="12" fill="#1a0b2e" opacity="0.52"/></pattern>
          ...
          <g id="vd-ship" />
          <text x="180" y="128" textAnchor="middle" fontSize="7" fontFamily="JetBrains Mono, monospace" fill="#c9ff2f" opacity="0.8">
            STAY IN THE MIDDLE
          </text>
        </svg>
      </div>
